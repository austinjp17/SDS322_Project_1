...
### Names: Wendi Deng (wd4737), Kevin Tran, Austin Palmer
...
**The dataset is tidy because each row is an individual observation (Asian American) and each variable (explanatory and outcome) has its own columns.There is also no rows with any NA values in any of the variables and there is only the six variable of interest present in the new dataset.**
...
```{r}
# Select variables for analysis of familiarities effect on QoL
social_dataset <- data |>
  select(English.Speaking, English.Difficulties, Identify.Ethnically, Familiarity.with.America, Belonging, Discrimination, Duration.of.Residency, US.Born)

working_dataset <- drop_na(social_dataset)
```
